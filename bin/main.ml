open Core
module V = Raycast.Vec3
module R = Raycast.Ray
module World = Raycast.World
module Sphere = Raycast.Sphere
module Hittable = Raycast.Hittable
module Hit_record = Raycast.Hit_record
module Interval = Raycast.Interval

let () =
  let world =
    World.make
      [ Sphere.to_hittable (Sphere.make (V.make 0. 0. (-1.)) 0.5)
      ; Sphere.to_hittable (Sphere.make (V.make 0.4 0.34 (-1.)) 0.2)
      ; Sphere.to_hittable (Sphere.make (V.make (-0.38) (-0.24) (-0.7)) 0.12)
      ; Sphere.to_hittable (Sphere.make (V.make 0. (-100.5) (-1.)) 100.)
      ]
  in
  let camera =
    Raycast.Camera.make
      ~aspect_ratio:(16. /. 9.)
      ~image_width:400
      ~samples_per_pixel:10
      ~max_depth:10
  in
  Raycast.Camera.render camera world
;;
